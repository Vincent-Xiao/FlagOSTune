#!/usr/bin/env python3
"""FlagGems vs CUDA kernel performance comparison analysis
...
Output:
- reports/{model_name}/perf_analysis.md
"""
...
def infer_aten_op(kernel_name: str) -> str:
    ...
    # High priority exact rules (match first)
    ...
    # Avoid substring false matches (e.g., multimem -> mul, scatter -> cat)
...
def write_excel_tables(excel_path: Path, tables: List[Dict[str, object]]) -> None:
    if not HAS_OPENPYXL:
        print(f"Warning: openpyxl not installed, skipping Excel output: {excel_path}")
        return
...
def build_compare_rows(
    ...
    for aten in ordered:
        ...
        # Only count kernels that both CUDA and FlagGems have calls
...
def main() -> None:
    ...
    out_md = reports_path / "perf_analysis.md"
    out_xlsx = reports_path / "perf_analysis.xlsx"
    out_shape_md = reports_path / "shape_analysis.md"
    out_shape_xlsx = reports_path / "shape_analysis.xlsx"
    ...
    report: List[str] = []
    report.append("# FlagGems and CUDA Operator Library Performance Comparison")
    report.append("")

    report.append("## CUDA kernels (sorted by total time)")
    report.append("")
    cuda_headers = ["Framework Op", "Execution Op", "Calls", "Total Time(ms)", "Avg Time(us)", "%"]
    ...
    report.append("## FlagGems kernels (sorted by total time)")
    report.append("")
    gems_headers = ["Framework Op", "Execution Op", "Calls", "Total Time(ms)", "Avg Time(us)", "%"]
    ...
    report.append("## CUDA vs FlagGems kernel comparison (sorted by CUDA total time)")
    report.append("")
    ...
    report.append("## CUDA vs FlagGems kernel comparison (sorted by speedup descending)")
    report.append("")
    ...
    write_excel_tables(
        out_xlsx,
        [
            {"sheet_name": "CUDA by total time", "headers": cuda_headers, "rows": cuda_rows},
            {"sheet_name": "FG by total time", "headers": gems_headers, "rows": gems_rows},
            {"sheet_name": "Compare by CUDA total", "headers": compare_headers, "rows": compare_cuda_rows},
            {"sheet_name": "Compare by speedup", "headers": compare_headers, "rows": compare_speedup_rows},
        ],
    )
    ...
    shape_report.append("# CUDA and FlagGems aten::mm Input Shape Analysis")
    shape_report.append("")
    shape_report.append("## CUDA aten::mm operator (sorted by total time)")
    shape_report.append("")
    shape_report.append(
        md_table(
            ["Framework Op", "Execution Op", "Input Shape", "CUDA Calls", "CUDA Total(ms)", "CUDA %"],
            cuda_mm_rows,
        )
    )
    shape_report.append("")

    shape_report.append("## FlagGems aten::mm operator (sorted by total time)")
    shape_report.append("")
    shape_report.append(
        md_table(
            ["Framework Op", "Execution Op", "Input Shape", "FG Calls", "FG Total(ms)", "FG %"],
            gems_mm_rows,
        )
    )
    shape_report.append("")
    ...
    shape_cuda_headers = ["Framework Op", "Execution Op", "Input Shape", "CUDA Calls", "CUDA Total(ms)", "CUDA %"]
    shape_gems_headers = ["Framework Op", "Execution Op", "Input Shape", "FG Calls", "FG Total(ms)", "FG %"]
    write_excel_tables(
        out_shape_xlsx,
        [
            {"sheet_name": "CUDA_mm_shape", "headers": shape_cuda_headers, "rows": cuda_mm_rows},
            {"sheet_name": "FG_mm_shape", "headers": shape_gems_headers, "rows": gems_mm_rows},
        ],
    )

    print(f"Report generated: {out_md}")
    print(f"Excel generated: {out_xlsx}")
    print(f"Shape analysis: {out_shape_md}")
    print(f"Shape Excel: {out_shape_xlsx}")
